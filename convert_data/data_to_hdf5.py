...
def generate_hdf5_data(dataset, path, num_files=4):
    ...
    dataloader = torch.utils.data.DataLoader(
        dataset, num_workers=32, batch_size=1, collate_fn=collate_fn
    )
    ...
    for i, (image, label) in enumerate(dataloader):
        ...
        if i == 0:
            ...
            # Create a dataset in the file
            div = num_files[0]
            im_dataset = file.create_dataset(
                "images", (div, *image.shape), dtype=np.uint8
            )
            label_dataset = file.create_dataset("labels", (div,), dtype=np.uint8)
...
def imagenet10k_to_hdf5(
    num_files=1, save_encoded=False, resize=True, encoder_info=False
):
    output_path = "../data/imagenet10k/hdf52/"
    Path(output_path).mkdir(parents=True, exist_ok=True)
